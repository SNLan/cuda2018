...
{	
	...
	cudaMemcpy((void*)d_b,(void*)d_a ,memSize,cudaMemcpyDeviceToDevice);
	// Check for any CUDA errors
	checkCUDAError("cudaMemcpy calls2");
	// clear host memory
	for (n = 0; n < dimA; n++)
	{
		h_a[n] = 0.f;
	...
	cudaMemcpy((void*)h_a,(void*)d_b,memSize,cudaMemcpyDeviceToHost);

	// Check for any CUDA errors
		checkCUDAError("cudaMemcpy calls3");
