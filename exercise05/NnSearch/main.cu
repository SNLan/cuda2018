		...
		const Point &query)
{
			...
			if(s=='X'){
				if(fabs(points[kBack.PointIdx].x - query.x)<dist){
					...
					if(query.x <= points[kBack->PointIdx].x ){ // if query is in the left subspace of kBack, then go to the right subspace
						...
						indexTemp = findIndexbyPoint(&nodes,&points,&query);
						kSearch = nodes[indexTemp];
					}
					else{ // kSearch = kBack->left
						Point pointTemp = points[kBack->PointIdx+1];
						indexTemp = findIndexbyPoint(&nodes,&points,&query);
						kSearch = nodes[indexTemp];
					}

					if(kSearch !=NULL){
						search_path.push(kSearch);   // add k_search to search_path
					}
				}
				...
				if(fabs(points[kBack.PointIdx].y - query.y)<dist){
				// get perpendicular distance and decide whether we need to go opposite side
					if((nearest - query)>(points[kBack->PointIdx] - query)){
						nearest = points[kBack->PointIdx];
						dist = points[kBack->PointIdx] - query;
					...
					int indexTemp;
					if(query.y <= points[kBack->PointIdx].y ){ // if query is in the left subspace of kBack, then go to the right subspace
						//kSearch = kBack->right
						Point pointTemp = points[kBack->RightChildIdx];
						indexTemp = findIndexbyPoint(&nodes,&points,&query);
						kSearch = nodes[indexTemp];
	...
	int res = findIndexbyPoint(&nodes,&points,&nearest);
