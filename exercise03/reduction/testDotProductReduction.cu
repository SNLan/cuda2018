...
__global__ void reduceSumKernel(float *d_out, const float* d_in)
{
...
int main(int argc, char* argv[])
{
	...
	float* cpuResult;
	float* cpuTemp;
	cudaMallocHost((void**) &cpuTemp, sizeof(float));
	...
	for (int iter = 0; iter < 1000; ++iter)
	{
		...
		case 2:
			...
			int blocks;
			blocks = (dim+ MAX_THREADS-1) / MAX_THREADS;
		
			if(blocks>1){	
				reduceSumKernel<<<blocks,MAX_THREADS,MAX_THREADS*sizeof(float)>>>(gpuResult2,gpuResult1);
				reduceSumKernel<<<1, MAX_THREADS,MAX_THREADS*sizeof(float)>>>(d_temp, gpuResult2);
			}
			else{
				reduceSumKernel<<<1, MAX_THREADS,MAX_THREADS*sizeof(float)>>>(d_temp, gpuResult1);

			}

			cudaMemcpy(cpuTemp, d_temp,
					sizeof(float), cudaMemcpyDeviceToHost);
			finalDotProduct = cpuTemp[0];
			break;
