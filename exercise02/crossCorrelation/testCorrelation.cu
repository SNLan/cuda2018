...
// global texture reference (2D texture)
 ...
 */__global__ void correlationKernel(float3 *_dst, int _w, int _h)
{
	...
	for (int j = 0; j < _h; ++j)
	{
		for (int i = 0; i < _w; ++i)
		{
			...
			img1 =tex2D(texImg1, (float)i/(float)_w,(float)j/(float)_h);
			img2 =tex2D(texImg2, (float)(i+offsX)/(float)_w,(float)(j+offsY)/(float)_h);
...
int main(int argc, char* argv[])
{
    ...
    cudaMallocArray(&gpuTex1,&floatTex,w,h);
    cudaMallocArray(&gpuTex2,&floatTex,w,h);

    cudaMemcpyToArray(gpuTex1,0,0,img3,sizeof(float4)*w*h,cudaMemcpyHostToDevice);
    cudaMemcpyToArray(gpuTex2,0,0,img4,sizeof(float4)*w*h,cudaMemcpyHostToDevice);

    cudaBindTextureToArray(texImg1,gpuTex1,floatTex);
    cudaBindTextureToArray(texImg2,gpuTex2,floatTex);
    ...
    cudaUnbindTexture(texImg1);
    cudaUnbindTexture(texImg2);
