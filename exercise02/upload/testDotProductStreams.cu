	...
	for (int iter = 0; iter < 100; ++iter)
	{
		...
		case 2:
			...
			for (unsigned int pass = 0; pass < 2; pass++){
				// !!! missing !!!
				// Calculate the dot product with non-pageable memory.
				cudaMemcpyAsync(gpuOperator2[pass], cpuOperator2[pass],dim * sizeof(float), cudaMemcpyHostToDevice);

				// call the kernel
				dotProdKernel<<<blockGrid, threadBlock, 0, stream[pass]>>>(gpuResult[pass],
						gpuOperator1[pass], gpuOperator2[pass], dim);

				// download and combine the results of multiple threads
				cudaMemcpyAsync(cpuResult[pass], gpuResult[pass],MAX_BLOCKS * MAX_THREADS * sizeof(float),
						cudaMemcpyDeviceToHost);
			}


			for (unsigned int pass = 0; pass < 2; pass++){
				// TODO: Synchonize streams
				cudaStreamSynchronize(stream[pass]);
			}
			for (unsigned int pass = 0; pass < 2; pass++){
				// Calculate the result
				float finalDotProduct = 0.0f;
